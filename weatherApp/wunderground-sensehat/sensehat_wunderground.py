...
STATE = "MA"
CITY = "Topsfield"
SENSOR_LOCATION_NAME = "Basement"
#WUNDERGROUND_API_KEY = "c2ac867add89df72"
BUCKET_NAME = ":partly_sunny: " + CITY + " Weather"
BUCKET_KEY = "shwu1"
ACCESS_KEY = "ugFV8oDtRrt9AH0WaoMAg8SGHEOevZlG"
MINUTES_BETWEEN_READS = 7
METRIC_UNITS = False
# ---------------------------------
...
def main():
	...
	#streamer.log(":house: Location",conditions['current_observation']['display_location']['full'])
        ...
        while True:
            ...
            #Solar edge API
            solar = get_solar()
            if isFloat(solar['sitesOverviews']['siteEnergyList'][0]['siteOverview']['currentPower']['power']):
                streamer.log(":sunny: " + "Roof Array" + " Solar Power(W)",solar['sitesOverviews']['siteEnergyList'][0]['siteOverview']['currentPower']['power'])
            print("Solar power (W):" + str(solar['sitesOverviews']['siteEnergyList'][0]['siteOverview']['currentPower']['power']))

            #Meter Reading
            kWh = get_kWh()
            streamer.log(":sunny: " + "Electric Meter Reading(kWh) ", kWh)
            print("Meter Reading(kWh): " + str(kWh))
            ...
            #     streamer.log(":dash: " + CITY + " Wind Direction",wind_dir_icon(conditions)
            #     if METRIC_UNITS:
